...
>(({ agentId, className, waveformClassName, onConnect, onDisconnect, onError, onMessage, onSendMessage }, ref) => {
  ...
  useConversation({
    ...
    onMessage: (msg) => {
      if (msg.message) {
        onMessage?.({ source: msg.source === "user" ? "user" : "ai", message: msg.message })
      }
    },
    onError: (error: string | Error) => onError?.(typeof error === "string" ? new Error(error) : error),
  })
  ...
  return (
    <div
      ref={ref}
      className={cn("flex w-full items-end justify-end p-4", className)}
    >
